...
class ModchartState 
{
    ...
    function new()
    {
				...
				//Termination and Censory shit -Discussions
				Lua_helper.add_callback(lua,"kbAlertTOGGLE", function(toAdd:Bool) {
					PlayState.instance.KBALERT_TOGGLE(toAdd);
				});
				Lua_helper.add_callback(lua,"kbAttackTOGGLE", function(toAdd:Bool) {
					PlayState.instance.KBATTACK_TOGGLE(toAdd);
				});
				Lua_helper.add_callback(lua,"kbPincerPrepare", function(laneID:Int, goAway:Bool) {
					PlayState.instance.KBPINCER_PREPARE(laneID,goAway);
				});
				Lua_helper.add_callback(lua,"kbPincerGrab", function(laneID:Int) {
					PlayState.instance.KBPINCER_GRAB(laneID);
				});
				Lua_helper.add_callback(lua,"kbAttackAlert", function(pointless:Bool = false) {
					PlayState.instance.KBATTACK_ALERT(pointless);
				});
				Lua_helper.add_callback(lua,"kbAttackAlertDouble", function(pointless:Bool = false) {
					PlayState.instance.KBATTACK_ALERTDOUBLE(pointless);
				});
				Lua_helper.add_callback(lua,"kbAttack", function(prepare:Bool = false, sound:String = 'attack') {
					PlayState.instance.KBATTACK(prepare, sound);
				});
				Lua_helper.add_callback(lua,"dodgeTimingOverride", function(newValue:Float = 0.22625) {
					PlayState.instance.dodgeTimingOverride(newValue);
				});
				Lua_helper.add_callback(lua,"dodgeCooldownOverride", function(newValue:Float = 0.1135) {
