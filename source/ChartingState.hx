...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		if (PlayState.SONG.song != 'Termination' || PlayState.SONG.song != 'Termiante' ||
		PlayState.SONG.song != 'Power-Link' || PlayState.SONG.song != 'Final-Destination' || PlayState.SONG.song != 'Censory-Overload') {
		...
		if (PlayState.storyDifficulty == 2)
			...
			if (PlayState.SONG.song == 'Termination')
				difficulty = 'Very Unfair';	
			if (PlayState.SONG.song == 'Termiante')
				difficulty = 'Crashers';	
			if (PlayState.SONG.song == 'Power-Link' )
				difficulty = 'HARDER THAN USUAL';	
			if (PlayState.SONG.song == 'Final-Destination') 
				difficulty = 'Godly Impossible';	
			if (PlayState.SONG.song == 'Censory-Overload')
				difficulty = 'Demon';	
		}

		#if windows
		 // Updating Discord Rich Presence
		 DiscordClient.changePresence("In the Charting Menu | Charting Song: " + PlayState.SONG.song + " " + difficulty, null);
		#end
	...
	var noteTypes:Array<String> = ['Normal', 'Fire', 'Death', 'Warning', 'Angel', 'Alt Anim', 'Bob', 'Glitch', 'Warning New', 'Snow', 'DeathNew', 'Fire'];
	...
	private function addNote(?n:Note):Void
	{
		...
		if (FlxG.keys.pressed.ONE) {
			noteType = 8;
			trace('should place whatever type 14');
		}
		if (FlxG.keys.pressed.TWO) {
			noteType = 9;
			trace('should place whatever, I forgot type 12');
		}
		if (FlxG.keys.pressed.Z) {
			noteType = 10;
			trace('should place whatever type 13');
		}
