...
class Note extends FlxSprite
{
	...
	//custom new notes shit k?

	public var warningNew:Bool = false;
	public var snow:Bool = false;
	public var deathNew:Bool = false;
...
//	public var noteColors:Array<String> = ['purple', 'blue', 'green', 'red', 'white', 'yellow', 'violet', 'darkred', 'orange'];
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?noteType:Int = 0, ?_mustPress:Bool = false, ?inCharter:Bool = false)
	{
		...
		var noteOffsetY:Array<String> = CoolUtil.coolTextFile(Paths.txt('offsetForNoteY'));
		var noteOffset:Array<String> = CoolUtil.coolTextFile(Paths.txt('offsetForNoteX')); //shit testing 
	//	x = Std.parseFloat(noteOffset[2]);
		//man why doesn't shit just work

		/*var offsetForNote:Array<Int> = [
			4, //warning offset
			6, //snow offset
			9, //death offset
			5 //fire offset
	...
	*/
		...
		// noteType correction for custom new shit, god this is hard

		warningNew = noteType == 8;
		snow = noteType == 9;
		deathNew = noteType == 10;
		fire = noteType == 11;
	...
	*/
				...
				if (warningNew || snow || deathNew || fire)
				{
					switch (noteType)
					{
						case 8:
							frames = Paths.getSparrowAtlas('customNote/termination_NOTE_assets');
		
							animation.addByPrefix('greenScroll', 'attention green');
							animation.addByPrefix('redScroll', 'attention red');
							animation.addByPrefix('blueScroll', 'attention blue');
							animation.addByPrefix('purpleScroll', 'attention purple');
						...
						case 9:
							...
							animation.addByPrefix('greenScroll', 'Green Angel arrow');
							animation.addByPrefix('redScroll', 'Red Angel arrow');
							animation.addByPrefix('blueScroll', 'Blue Angel arrow');
							animation.addByPrefix('purpleScroll', 'Purple Angel arrow');
		
					//		x = 0;
							x = Std.parseFloat(noteOffset[0]);
							y = Std.parseFloat(noteOffset[0]);
							setGraphicSize(Std.int(width * 0.7));
							updateHitbox();
							antialiasing = true;
						case 10:
							...
							animation.addByPrefix('greenScroll', 'Green Death arrow');
							animation.addByPrefix('redScroll', 'Red Death arrow');
							animation.addByPrefix('blueScroll', 'Blue Death arrow');
							animation.addByPrefix('purpleScroll', 'Purple Death arrow');
		
				//			x = 0;
							x = Std.parseFloat(noteOffset[1]);
							y = Std.parseFloat(noteOffset[1]);
							setGraphicSize(Std.int(width * 0.7));
							updateHitbox();
							antialiasing = true;
						case 11:
