	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'bfWhitty':
						...
						animation.addByPrefix('singRIGHT', 'Whitty Sing Note LEFT', 24);
						animation.addByPrefix('singDOWN', 'Whitty Sing Note DOWN', 24);
						animation.addByPrefix('singLEFT', 'whitty sing note right', 24);

						animation.addByPrefix('dodge', 'Whitty Sing Note DOWN', 24);
		
						addOffset('idle', 50); //legit offsets stolen from og source code. its still public
