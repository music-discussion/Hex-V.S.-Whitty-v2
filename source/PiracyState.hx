...
class PiracyState extends MusicBeatState
{
	...
	private var crashing:FlxSprite;
	...
	override function create()
	{
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"You've been detected playing on HTML5. "
			+ "Because of this, there are a few downsides."
			+ "\nSome songs have things with only effect on desktop."
			+ "\n\nWhat's expected:\n\n"
			+ "Freezes on songs with sign posts, and terminate song crashing."
			+ "\n& more changes I might have missed"
			+ "\n\nPress Space to crash-Err!!!--terminate your game\nor ESCAPE to ignore this",
			32);
	...
	override function update(elapsed:Float)
	{
		if (controls.ACCEPT)
		{
			crashing.animation.play('justGiveUp');
		}
		if (controls.BACK)
		{
			leftState = true;
